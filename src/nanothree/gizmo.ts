...
function buildArrow(axis: 'x' | 'y' | 'z', length: number, tipSize: number): { positions: number[], indices: number[] } {
  const l = length
  const t = tipSize
  ...
  tip1[ai] = l - t; tip1[perp1] = t * 0.4
  tip2[ai] = l - t; tip2[perp1] = -t * 0.4

  // Also add a perpendicular pair for the other axis
  const tip3 = [0, 0, 0]
  const tip4 = [0, 0, 0]
  tip3[ai] = l - t; tip3[perp2] = t * 0.4
  tip4[ai] = l - t; tip4[perp2] = -t * 0.4

  positions.push(
    0, 0, 0,          // 0: origin
    end[0], end[1], end[2],     // 1: tip
    tip1[0], tip1[1], tip1[2],  // 2
    tip2[0], tip2[1], tip2[2],  // 3
    tip3[0], tip3[1], tip3[2],  // 4
    tip4[0], tip4[1], tip4[2],  // 5
  )

  indices.push(
    0, 1,  // shaft
    1, 2,  // arrowhead prong 1
    1, 3,  // arrowhead prong 2
    1, 4,  // arrowhead prong 3
    1, 5,  // arrowhead prong 4
  )
...
function buildCircle(axis: 'x' | 'y' | 'z', radius: number, segments: number): { positions: number[], indices: number[] } {
  const positions: number[] = []
  const indices: number[] = []
...
function buildScaleHandle(axis: 'x' | 'y' | 'z', length: number, boxSize: number): { positions: number[], indices: number[] } {
  const positions: number[] = []
  const indices: number[] = []
  const ai = axis === 'x' ? 0 : axis === 'y' ? 1 : 2
  const perp1 = (ai + 1) % 3
  ...
  const bs = boxSize / 2
  const cx = end[0], cy = end[1], cz = end[2]
  const offsets = [
  ...
  for (const off of offsets) {
    const v = [cx, cy, cz]
    v[perp1] += off[0]; v[perp2] += off[1]; v[ai] += off[2]
